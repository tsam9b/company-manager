...
const submit = async () => {
	form.clearErrors();
	try {
		await window.axios.post('/employee', form.data());
		success(t('employee.created'));
		...
		errorToast(t('employee.createError'));
	}
};
...
<template>
	<Head :title="t('employee.createTitle')" />

	<AuthenticatedLayout>
		<template #header>
			<h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
				{{ t('employee.createTitle') }}
			</h2>
		</template>
